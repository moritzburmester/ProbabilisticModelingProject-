...
from bnn import BNNLeNet

# CUDA settings
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

def getModel(net_type, inputs, outputs, priors, activation_type):
    if (net_type == 'lenet'):
        return BNNLeNet(outputs, inputs, priors, activation_type)
...
def run(dataset, net_type):
    ...
    net = getModel(net_type, inputs, outputs, priors, activation_type).to(device)
