...
def test_single_image(model, img_path, dataset_name, num_classes=10):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
...
def rotate_img(x, deg, img_size, channels):
    rotated_img = np.zeros((channels, img_size, img_size))
    for c in range(channels):
        rotated_channel = nd_rotate(x[c].reshape(img_size, img_size), deg, reshape=False)
        rotated_img[c] = rotated_channel
    return rotated_img
...
def rotating_image_classification(model, img, threshold=0.5, num_classes=10, selected_classes=None,
    ...
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    Mdeg = 180
    num_steps = int(Mdeg / 10) + 1
    ldeg = []
    lp = []
    lu = []
    classifications = []

    img_size = img.shape[-1]  # Get the size of the image (28 for MNIST/FashionMNIST, 32 for CIFAR10)
    channels = img.shape[0]  # Get the number of channels (1 for MNIST/FashionMNIST, 3 for CIFAR10)
    nimgs = np.zeros((channels, img_size, img_size * num_steps))
    ...
    for i, deg in enumerate(np.linspace(0, Mdeg, num_steps)):
        nimg = rotate_img(np.array(img), deg, img_size, channels).reshape(channels, img_size, img_size)
        nimg = np.clip(a=nimg, a_min=0, a_max=1)
        nimgs[:, :, i * img_size: (i + 1) * img_size] = nimg
        nimg = np.transpose(nimg, (1, 2, 0))  # Change from (C, H, W) to (H, W, C) for normalization
    ...
    nimgs = np.clip(nimgs, 0, 1)

    # Adjust for RGB or grayscale images
    if channels == 1:
        ax1.imshow(nimgs[0], cmap='gray', aspect='equal')
    else:
        ax1.imshow(np.transpose(nimgs, (1, 2, 0)), aspect='equal')
    ax1.axis('off')
...
def noisy_image_classification(model, img, threshold=0.5, num_classes=10, selected_classes=None,
    ...
    for i, std_dev in enumerate(np.linspace(0, max_std_dev, num_steps)):
        nimg = add_gaussian_noise(np.array(img), std_dev).reshape(channels, img_size, img_size)
        nimg = np.clip(a=nimg, a_min=0, a_max=1)
        nimgs[:, :, i * img_size: (i + 1) * img_size] = nimg
        nimg = np.transpose(nimg, (1, 2, 0))  # Change from (C, H, W) to (H, W, C) for normalization
        img_tensor = trans(nimg).unsqueeze(0).to(device)  # Ensure normalization and move to device
    ...
    # Denormalize images before plotting
    nimgs_denorm = denormalize(torch.tensor(nimgs), mean, std).numpy()

    # Adjust for RGB or grayscale images
    if channels == 1:
        ax1.imshow(1 - nimgs_denorm[0], cmap='gray', aspect='equal')
    else:
        ax1.imshow(np.transpose(1 - nimgs_denorm, (1, 2, 0)), aspect='equal')
    ax1.axis('off')
